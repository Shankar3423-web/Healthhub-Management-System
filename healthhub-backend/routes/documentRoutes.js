...
router.post('/upload', auth, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (appointmentId) {
      ...
      appointment = await Appointment.findOne({ patientId: patient.patientId, status: 'Booked' })
        .sort({ date: -1 })
        .populate('doctorId', 'name');
    }

    if (!appointment) return res.status(400).json({ msg: 'Appointment not found or invalid.' });
...
router.get('/my', auth, async (req, res) => {
  try {
    ...
    const docs = await Document.find({ patientId: patient.patientId })
