...
router.get('/my', auth, async (req, res) => {
  try {
    ...
    // Return ALL prescriptions for this patient, regardless of appointment status
...
// ✅ NEW: Mark prescription as completed (Patient Action)
router.put('/complete/:id', auth, async (req, res) => {
  try {
    ...
    res.json({ msg: 'Prescription marked as completed', prescription });
  } catch (err) {
    console.error('Prescription completion error:', err.message);
    res.status(500).json({ msg: 'Server error' });
  }
});
